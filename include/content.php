...
if (isset($_GET['tb'])) {
	...
	switch ($tb) {
		case 'ok':
			...
			break;
		case 'loi':
			echo '<script>alert("Cập nhật thông tin lỗi! Vui lòng kiểm tra lại thông tin!")</script>';
			break;
		case 'ok1':
			echo '<script>alert("Đăng ký thành công. Nhân viên sẽ thông báo sau!!!")</script>';
			break;
		case 'ok2':
			echo '<script>alert("Đăng ký trả phòng thành công! Hãy chờ cán bộ duyệt!!!")</script>';
			break;
		case 'loi1':
		...
		case 'loi2':
			echo '<script>alert("Vui lòng kiểm tra lại thông tin phòng bạn đã kết thúc hợp đồng, chưa được cán bộ duyệt hoặc chưa đăng ký phòng!!!")</script>';
			break;
		case 'loi3':
			$sn = $_GET['sn'];
...
if (isset($_GET['action'])) {
	...
	switch ($action) {
		...
		case 'capnhapthongtin':
			...
			break;
		case 'thongtin':
				include_once('view/thongtincanhan.php');
				break;
		case 'dkphong':
